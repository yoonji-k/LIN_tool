...
def random_injection():
    for i in range(1000):
        ...
        data = '\t'.join([f"0x{d:02x}" for d in rd_attack.Data]) # data 확인용

        data_sum = rd_attack.FrameId
        for byte in rd_attack.Data:
            data_sum += byte
            if data_sum > 0xFF:
                data_sum -= 0xFF  # 255를 빼주기

        rd_checksum = calculate_checksum(data_sum)
        rd_attack.Checksum = rd_checksum


        wResult = LIN.Write(hClient, hHw, rd_attack)
        if wResult == TLIN_ERROR_OK:
            print("ID:", f"0x{rd_attack.FrameId:02X}", "Data:", data, "Checksum:", f"0x{rd_attack.Checksum:02X}")
        else:
            print("Error: ", wResult)
