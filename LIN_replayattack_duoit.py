...
with open(filename, 'r', encoding='cp949') as f:
    ...
    for line in f:
        ...
        try:
            msg_id = parts[2].strip().upper()
            if msg_id == '08':
                data = [int(parts[i], 16) for i in range(4, 9)]
            else:
                data = [int(parts[i], 16) for i in range(4, 6)]

            if msg_id in periods:
                period = periods[msg_id][period_index[msg_id]]
                # 인덱스 번갈아가면서 적용
                period_index[msg_id] = 1 - period_index[msg_id]
            else:
                period = 0.100  # 기본 주기 100ms
