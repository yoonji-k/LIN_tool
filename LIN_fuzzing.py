...
# LIN 메시지 전송
def random_fuzzing_injection():
    for i in range(5000):
        msg_id = random.randint(0x00, 0x3F)  # LIN 메시지 ID 범위 (1~63)
        ...
        print(f"Sending Fuzzing Message - ID: 0x{msg_id:02X}, Data: {hex_data}")
        ...
        if result != TLIN_ERROR_OK:
            print(f"Error sending Fuzzing message. Error code: {result}")

        time.sleep(0.1)  # 메시지 간 간격 (100ms)

# 랜덤 Fuzzing 공격 수행
random_fuzzing_injection()
